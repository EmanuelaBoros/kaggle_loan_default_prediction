...
def select_from_all_data_regression(max_features ,train, test, train_loss, train_loss_array,  column_names, run, target):    
...
#    initial_auc = 0.942227469693
    ...
    if os.path.isfile("data/" + target + "_max_generation_dict_run_" + str(run) + ".pkl"):
        print "Loading saved values "
        max_pearson_generation_dict = pickle.load(open("data/" + target + "_max_generation_dict_run_" + str(run) + ".pkl", "rb" ) )
        best_features_regression_generation_dict = pickle.load(open("data/best_features_regression_ " + target + "_generation_dict_run_" + str(run) + ".pkl", "rb" ) )
...
#                    print "MAE :" + str(mae)      
                ...
                if  average_pearson >  max_pearson:
                    ...
                    if target == "spearman":
                        print "Mean Spearman = " + str(average_pearson)
        ...
        pickle.dump( max_pearson_generation_dict, open( "data/" + target + "_max_generation_dict_run_" + str(run) + ".pkl", "wb" ) )
        pickle.dump( best_features_generation_dict, open("data/best_features_regression_ " + target + "_generation_dict_run_" + str(run) + ".pkl", "wb" ) )       
...
#    test = pandas.read_csv('data/test_v2.csv')
    ...
    run = 2
    target = "spearman"
    select_from_all_data_regression(max_features,train, test, train_loss,train_loss_array, column_names, run, target)
