...
########## Predict SVC ################       
#        clf = SVC(C=1e2, kernel='rbf', degree=2, gamma=0.0, coef0=0.0, shrinking=True, probability=False, tol=0.001, cache_size=200, class_weight=None, verbose=False, max_iter=-1, random_state=None)
#        clf.fit(X_train_split,y_train_split)      
#        probas_ = clf.predict(X_test_split)
#        print probas_
#        
##        print "... " + str(len(y_test_split_initial)) + " " + str(len(probas_))
#        
##        prediction_proba = probas_[:,1]
#        
#        predictionIndexes0 = np.where(probas_ < 0.5)[0]
#        predictionIndexes1 = np.where(probas_ > 0.5)[0]  
#        print sum(probas_)
#        
##########         ################      
...
#            print p
        predicted_regr_values_X = [[p1, p2, p3] for p1, p2, p3 in zip(predicted_regr_values_025, predicted_regr_values_05, 
                                   predicted_regr_values_075)]
